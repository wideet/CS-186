...
public class TestOptimalQueryPlan {
  ...
  @Test
  @Category(StudentTestP4.class)
  public void studentTest1() throws DatabaseException, QueryPlanException {
    Database.Transaction transaction = this.database.beginTransaction();
    ...
    while (recordIterator.hasNext()) {
      ...
      assertTrue(values.get(2) instanceof StringDataBox);

      assertTrue(values.get(1).getInt() > 0);
      assertTrue(values.get(0).getInt() > 0);
    }
  }

  @Test
  @Category(StudentTestP4.class)
  public void studentTest2() throws DatabaseException, QueryPlanException {
    Database.Transaction transaction = this.database.beginTransaction();
    QueryPlan queryPlan = transaction.query(this.defaulTableName);

    queryPlan.select("int", QueryPlan.PredicateOperator.LESS_THAN_EQUALS, new IntDataBox(0));
  ...
  @Test
  @Category(StudentTestP4.class)
  public void studentTest8() throws DatabaseException, QueryPlanException {
    Database.Transaction transaction = this.database.beginTransaction();
    QueryPlan queryPlan = transaction.query(this.defaulTableName);
