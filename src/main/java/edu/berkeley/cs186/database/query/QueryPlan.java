...
public class QueryPlan {
  ...
  private QueryOperator pushDownSelects(QueryOperator source, int except) throws QueryPlanException, DatabaseException {
    /* TODO: Implement me! */
    List<PredicateOperator> copyOperators = this.selectOperators;
    List<String> copyColumnNames = this.selectColumnNames;
    List<DataBox> copyDataBoxes = this.selectDataBoxes;
    if (except >= 0) {
      this.selectOperators.remove(except);
      this.selectColumnNames.remove(except);
      this.selectDataBoxes.remove(except);
    }
    this.finalOperator = source;
    this.addSelects();
    this.selectColumnNames = copyColumnNames;
    this.selectOperators = copyOperators;
    this.selectDataBoxes = copyDataBoxes;
    return this.finalOperator;
  }
  ...
  private QueryOperator minCostSingleAccess(String table) throws DatabaseException, QueryPlanException {
    ...
    // TODO: Implement me!
    SequentialScanOperator seqScan = new SequentialScanOperator(this.transaction, table);
    int seqScanCost = seqScan.getIOCost();
    ...
    int minIOCost = seqScanCost;
    for (int i : selectIndices) {
      indexScan = new IndexScanOperator(this.transaction, table,
              selectColumnNames.get(i), selectOperators.get(i), selectDataBoxes.get(i));
      int indexScanCost = indexScan.getIOCost();
      if (indexScanCost < minIOCost) {
        minSelectIdx = i;
        minIOCost = indexScanCost;
      }
    }
    if (minSelectIdx >= 0) {
      minOp = new IndexScanOperator(this.transaction, table, selectColumnNames.get(minSelectIdx),
              selectOperators.get(minSelectIdx), selectDataBoxes.get(minSelectIdx));
    } else {
      minOp = seqScan;
    }
  ...
  private QueryOperator minCostJoinType(QueryOperator leftOp,
                                        ...
                                        String rightColumn) throws QueryPlanException,
                                                                   DatabaseException {
    QueryOperator minOp = null;
    int minIOCost = Integer.MAX_VALUE;
    /* TODO: Implement me! */
    JoinOperator joinOp = new SNLJOperator(leftOp, rightOp, leftColumn, rightColumn, this.transaction);
    minIOCost = joinOp.getIOCost();
    minOp = joinOp;
    joinOp = new BNLJOperator(leftOp, rightOp, leftColumn, rightColumn, this.transaction);
    if (joinOp.getIOCost() < minIOCost) {
      minIOCost = joinOp.getIOCost();
      minOp = joinOp;
    }
    joinOp = new PNLJOperator(leftOp, rightOp, leftColumn, rightColumn, this.transaction);
    if (joinOp.getIOCost() < minIOCost) {
      minIOCost = joinOp.getIOCost();
      minOp = joinOp;
    }
    joinOp = new GraceHashOperator(leftOp, rightOp, leftColumn, rightColumn, this.transaction);
    if (joinOp.getIOCost() < minIOCost) {
      minIOCost = joinOp.getIOCost();
      minOp = joinOp;
    }
  ...
  private Map<Set, QueryOperator> minCostJoins(Map<Set, QueryOperator> prevMap,
    ...
    Map<Set, QueryOperator> map = new HashMap<Set, QueryOperator>();
    /* TODO: Implement me! */
    for (Set currTables : prevMap.keySet()) {
      for (int i = 0; i < joinTableNames.size(); i++) {
        for (Set newTable : pass1Map.keySet()) {
          Set tempSet = new HashSet(newTable);
          tempSet.retainAll(currTables);
          String leftTableName = joinLeftColumnNames.get(i).split("\\.")[0];
          if (tempSet.isEmpty() && newTable.contains(joinTableNames.get(i)) && currTables.contains(leftTableName)) {
            QueryOperator joinOp = minCostJoinType(prevMap.get(currTables), pass1Map.get(newTable),
                    joinLeftColumnNames.get(i), joinRightColumnNames.get(i));
            Set joinSet = new HashSet(currTables);
            joinSet.addAll(newTable);
            if (map.containsKey(joinSet)) {
              if(joinOp.getIOCost() < map.get(joinSet).getIOCost()) {
                map.put(joinSet, joinOp);
              }
            } else {
              map.put(joinSet, joinOp);
            }
