...
public class IntHistogram implements Histogram<Integer>{
  ...
  public float computeReductionFactor(PredicateOperator predicate,
    ...
    switch (predicate) {
      case EQUALS:
        return (1 / (float) this.getNumDistinct());

      case GREATER_THAN:
        return ((this.getMaxValue() - value.getInt()) / (float) (this.getMaxValue() - this.getMinValue()));

      case LESS_THAN:
        return ((value.getInt() - this.getMinValue()) / (float) (this.getMaxValue() - this.getMinValue()));

      case NOT_EQUALS:
        return (this.getNumDistinct() - 1) / (float) (this.getNumDistinct());

      case LESS_THAN_EQUALS:
        return ((value.getInt() - this.getMinValue() + 1) / (float) (this.getMaxValue() - this.getMinValue()));

      case GREATER_THAN_EQUALS:
        return ((this.getMaxValue() - value.getInt() + 1) / (float) (this.getMaxValue() - this.getMinValue()));
    }
    return 0;
  }
